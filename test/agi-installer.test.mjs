...
test('installMaintainer passes release tag into the install plan', () => {
  ...
  assert.equal(
    result.packageSpec,
    'https://github.com/flonest-app/agentic-ai-skills/releases/download/v0.1.14/agentic-ai.tgz',
  );
});
...
test('installer can fall back to GitHub asset API downloads', () => {
  const cacheDir = mkdtempSync(join(tmpdir(), 'agentic-ai-cache-'));
  const packageSpec = buildGitHubReleasePackageSpec({ tag: 'v0.1.14' });
  const calls = [];
  const downloaded = prepareNpmPackageSpec({
    packageSpec,
    cacheDir,
    ...
    spawnSyncImpl: (command, args, options = {}) => {
      ...
      if (String(url).includes('/releases/tags/v0.1.14')) {
        return {
          status: 0,
